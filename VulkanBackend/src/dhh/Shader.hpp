...
namespace dhh::shader
{
	...
	inline std::filesystem::path findShaderDirectory()
	{
		...
		while (true)
		{
			...
			{
				throw std::exception("Failed to find shaders directory");
			}
		}
	}
	...
	public:
		...
		std::vector<VkVertexInputBindingDescription> getVertexInputBindingDescription() const
		{
			if (type != Vertex)
			{
				throw std::exception("Need vertex shader");
			}
	...
	private:
		...
		DescriptorInfo reflect_descriptor(const spirv_cross::CompilerReflection& compiler,
		...
		static ShaderType getShaderType(const std::filesystem::path& FilePath)
		{
			...
			{
				throw std::exception("Cannot determine shader type");
			}
